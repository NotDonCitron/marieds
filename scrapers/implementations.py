...
class EbayScraper(BaseScraper):
    ...
    async def scrape(self, query: str) -> list[ListingResult]:
        ...
        # sacat 139971 is Consoles
        params = {
            "_nkw": query,
            "_sacat": "139971",
            "LH_ItemCondition": "3000|7000", # Used
            "_sop": "15", # Newly listed
            "_ipg": "60"
        }
        resp = await client.get(f"{self.base_url}/sch/i.html", params=params)
        soup = BeautifulSoup(resp.text, "lxml")
        results = []
        # Robustere Selektoren für eBay
...
class KleinanzeigenScraper(BaseScraper):
    ...
    async def scrape(self, query: str) -> list[ListingResult]:
        url = f"{self.base_url}/s-{query.replace(' ', '-')}/k0"
...
class AllegroScraper(BaseScraper):
    ...
    async def scrape(self, query: str) -> list[ListingResult]:
        # Map German/English queries to Polish for better results on Allegro
        pl_queries = {
        ...
        url = f"{self.base_url}/listing?string={search_query.replace(' ', '%20')}"
        try:
            html = await self._stealth_get(url, wait_ms=5000)
        except Exception as e:
            # Fallback for timeouts: try again with a simpler query
        ...
        for item in items:
            try:
                ...
                # Check for "czarn" (black) in title
...
class VintedScraper(BaseScraper):
    ...
    async def scrape(self, query: str) -> list[ListingResult]:
        # Vinted is extremely hard to scrape without stealth
        url = f"{self.base_url}/catalog?search_text={query.replace(' ', '%20')}"
        html = await self._stealth_get(url, wait_ms=5000)
        soup = BeautifulSoup(html, "lxml")
        results = []
        
        # Vinted item selectors
        items = soup.select("[data-testid^='product-item']")
        for item in items:
            try:
                # Vinted titles are often in alt tags of images or nested spans
